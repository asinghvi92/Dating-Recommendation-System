...
def training(training_files_path, ML_algo):
    #input: user vectors for 15,000 users 
    #output: final feature weights using regression 
    
    #steps:
    #1) generate user-user matrix : features and labels 
    #2) run ml: find weights using evaluation 
    
    #files :
    #temp.dict: temp training file - ask saket for actual 
    #compatibility_Score.py: computes algorithm generated final score for 2 users 
    #features: genearted from feature.py 
    #labels:generaetd from proxylabel, TODO: cosnider intiital filtering before creating "labels " of supervised set
    ...
    for userXID in dictionary:
        ...
        for userYID in dictionary:
            ...
            if dictionary[userXID] and dictionary[userYID]:
                ...
                if ML_algo==0:
                    if computed_score>=0.5:     #TODO:cahnge change change change 
...
def MLLabelAssignment(userX, userY ):
    ...
    feature_vector= np.array(featureXYComputation(userX, userY))
    feature_vector= feature_vector.reshape(1,-1)
    #print(feature_vector)
    
    #load classifier:
    with open('myClassifier.log', 'rb') as fid:
        logistic_classifier = cPickle.load(fid)
    result= logistic_classifier.predict(feature_vector)
    #print(result)
    return result

def run_training(sample_num, ML_algo=0):
    global samples_num
